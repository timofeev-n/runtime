...
inline void setFlagsOnce(std::atomic_uint32_t& bits, uint32_t mask)
{
	...
	do
	{
		...
		Assert2((value & mask) == 0, "Flags already set");
	}
...
{
	return CoreTaskSize + ::Runtime::alignedSize(dataSize, DefaultAlign);
}
...
{
	Verify(m_refs.fetch_add(1ui32) > 0);
}

...
struct INTERFACE_API IAnything
{
	...
	T as()
	{
		...
		else
		{
			...
			Verify(ptr != nullptr);// , "Typecast failure: [{0}] -> [{1}]", typeid(this), typeid(T));
...
#define KeepRC(rc_)\
	...
	static_assert(std::is_base_of_v<::Runtime::IRefCounted, std::remove_reference_t<decltype(rc_)>>, "must be a Runtime::IRefCounted");\
	\
	const ::Runtime::Com::RefCountedGuard ANONYMOUS_VARIABLE_NAME(rcGuard__) {static_cast<::Runtime::IRefCounted&>(rc_)}\

...
public:
	...
	explicit WeakComPtr(const ComPtr<T>& ptr)
	{
		if (ptr)
		{
			m_weakRef = Detail::asRefCounted(*ptr).getWeakRef();
		}
	}
	...
	ComPtr<T> acquire()
	{
		...
		Assert2(targetInstance, "RefCounted object acquired through weak reference, but instance doesn't provide target interface");

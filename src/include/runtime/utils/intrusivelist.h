...
public:
	...
	struct iterator
	{
		...
		iterator& operator++()
		{
			Assert2(node != nullptr, "can not be increment iterator");
		...
		iterator& operator--()
		{
			Assert2(node != nullptr, "can not be decrement iterator");
			Assert2(node->m_prev != nullptr, "can not be decrement iterator");
		...
		{
			Assert2(node != nullptr, "intrusive list iterator can not be dereferenced");
...
{
	Assert2(m_tail != nullptr, "list is empty (or invalid state)");
...
{
	Assert2(m_head != nullptr, "list is empty (or invalid state)");
...
{
	Assert2 (pos.node == nullptr || pos.node->m_list == this, "Invalid iterator");
...
{
	Assert2(first.node != nullptr, "Iterator can not be dereferenced");

	Assert2(first.node->m_list == this, "Invalid list reference. Possible element pointed by iterator already removed from list.");
	...
	else
	{
		...
		Assert2(last.node != nullptr, "Iterator can not be dereferenced");

		Assert2(last.node->m_list == this, "Invalid list reference. Possible element pointed by iterator already removed from list.");
		...
		Assert2(lastStaysAfterFirst, "The last iterator doesn't stay before the first one");
...
{
	Assert2(pos.node != nullptr, "Iterator can not be dereferenced");
	Assert2(pos.node->m_list == this, "Invalid list reference. Possible element pointed by iterator already removed from list.");
...
typename IntrusiveList<T>::Node& IntrusiveList<T>::asFreeNode(T& element)
{
	...
	Assert2(node.m_next == nullptr && node.m_prev == nullptr && node.m_list == nullptr, "Node already within list");
...
template<typename T>
decltype(auto) begin(Runtime::IntrusiveList<T>& container)
{
...
template<typename T>
decltype(auto) begin(const Runtime::IntrusiveList<T>& container)
{
	return container.begin();
}
...
template<typename T>
decltype(auto) end(Runtime::IntrusiveList<T>& container)
{
...
template<typename T>
decltype(auto) end(const Runtime::IntrusiveList<T>& container)
{
	return container.end();
}

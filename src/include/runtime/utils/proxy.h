...
decltype(auto) proxyInvoke(const ProxyClass_& proxy, P&& ... p) noexcept(Meta::FunctionTypeInfo<decltype(Func)>::NoExcept)
{
	...
	if constexpr (std::is_same_v<decltype(method), std::false_type>)
	{
		...
		const auto errorMessage = Runtime::strfmt("Interface [%1] does not expose method meta information [%2]", typeid(Class_), typeid(Func));
...
#define PROXY_INVOKE(Member, ...) Runtime::proxy::proxyInvoke<&Member>(static_cast<const ProxyClass__&>(*this), __VA_ARGS__)


#define CLASS_PROXY(Proxy_) \
	Runtime::proxy::ProxyImplementation<Proxy_>{}


#define DECLARE_PROXY(Interface, Proxy_)\
ATTRIBUTE_VALUE(Interface, Runtime::proxy::ProxyImplementationAttribute, Runtime::proxy::ProxyImplementation<Proxy_>{})

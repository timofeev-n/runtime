...
namespace Runtime::StringsDetail {
...
struct SplitSequence
{
	...
	public:
		...
		iterator& operator++()
		{
			if (m_current = StringsDetail::splitNext(m_str, m_current, m_separators); m_current.empty())
			{
				m_str = {};
				m_separators = {};
			}
	...
	{
		std::basic_string_view<C> next = StringsDetail::splitNext(str, {}, separators);
...
{
	return StringsDetail::split__(std::basic_string_view{text}, std::basic_string_view{separators});
}
...
#define TYPED_STR(T,text) Runtime::StringsDetail::choose<T>(L##text, text)
#define TYPED_CHR(T,chr) Runtime::StringsDetail::choose<T>(L##chr, chr)

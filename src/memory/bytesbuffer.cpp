...
void BufferStorage::reallocate(std::byte*& buffer, size_t size)
{
	...
	Assert(header.refs == 1);
	//Assert((oldHeader.flags & ReadOnlyFlag) != ReadOnlyFlag)
...
BytesBuffer BufferStorage::bufferFromClientData(std::byte* ptr, std::optional<size_t> size)
{
	...
	//Assert(!size || *size <= allocatedSize(storage), "Specified size ({0}) does not fit into buffer. bufferFromClientData can not grow buffer size")
	Assert(!size || *size <= allocatedSize(storage));
...
BufferBase::BufferBase(std::byte* data_): m_storage(data_)
{
	...
	Assert(!m_storage || header().refs > 0);
}
...
{
	Assert(!buffer || BufferUtils::refsCount(buffer) == 1);
	...
	, m_size(size_ ? *size_ : m_buffer.size() - offset_)
{
	if (!m_buffer)
	{
		Assert2(m_offset == 0 && (!size_ || *size_ == 0), "Invalid parameters while construct BufferView from empty ReadOnlyBuffer");
	}
	else
	{
		Assert(m_offset + m_size <= m_buffer.size());
	}
}
...
{
	return Runtime::refsCount(buffer.m_storage);
}
...
BytesBuffer BufferUtils::copy(const BufferBase& source, size_t offset, std::optional<size_t> size)
{
	...
	Assert(!size || offset + *size <= source.size());

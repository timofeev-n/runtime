...
{
	Assert2(m_stream, "Stream not initialized");
	return (m_stream);
}
...
Task<BytesBuffer> UvStream::read()
{
	...
	if (m_isDisposed.load() || !m_stream) {
		if (!m_buffer) {
			LOG_WARN("Attempt to read, but stream is closed");
		}

		co_return std::move(m_buffer);
	}

	Assert2(!m_readAwaiter, "::read operation already processed");
			...
			DEBUG_NOEXCEPT_Guard
			{
				...
				if (nread < 0)
				{
					if (const int reason = static_cast<int>(nread); reason != UV__EOF)
					{
						...
						LOG_DEBUG(Core::Format::format("UV Stream stream read error: ({})", uvErrorMessage(reason)));
					}

...
static testing::AssertionResult checkBufferDefaultContent(const T& buffer, size_t offset = 0, size_t contentOffset = 0, std::optional<size_t> size = std::nullopt)
{
	...
	Verify(offset + fillSize <= buffer.size());
	...
	for (size_t i = offset; i < fillSize; ++i)
	{
		...
		{
			return testing::AssertionFailure() << Core::Format::format("Value mismatch at position [{}]. Expected [{} , {}], but [{}, '{}']", i, static_cast<int>(expectedValue), expectedValue, static_cast<int>(bufferValue), bufferValue);
		}
	}

...
#include "../helpers/buffertestutils.h"
#include <runtime/network/socketcontrol.h>
// #include <runtime/network/websocketcontrol.h>
...
struct StreamTestParameter
{
	...
	StreamTestParameter(size_t clientsCount_, size_t blocksCount_, Runtime::Byte blockSize_, milliseconds sendingDelay_, milliseconds receiveDelay_)
...
TEST_P(Test_Network_StreamBasic, BrokenRead)
{
	...
	auto clientTask = [&]() -> Task<>
	{
		...
		if (buffer.size() > 0)
		{
			throw Excpt_("Expected empty buffer");
		}
	}();
